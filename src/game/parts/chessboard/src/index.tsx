...
export const Chessboard = forwardRef<ClearPremoves, ChessboardProps>((props, ref) => {
  const { customDndBackend, customDndBackendOptions, onBoardWidthChange, ...otherProps } = props;
  ...
  const metrics = useMemo(() => boardRef.current?.getBoundingClientRect(), [boardRef.current]);
  ...
  return (
    <ErrorBoundary>
      ...
      >
        ...
        <ChessboardDnDRoot customDndBackend={customDndBackend} customDndBackendOptions={customDndBackendOptions}>
          {boardWidth && (
